...
class RiemannianMetric(object):
    """
    Class for Riemannian metrics.
    """
    def __init__(self):
        print('Riemannian Metric Class')
        pass

    def hello(self):
        print('hello !')

    def hvsplit_R3(self, c):
        '''
        by Alice Le BRIGANT
    ...
    def geod_L2_R3(self, gamma_i, gamma_f, m):
        ''' 
        by Alice Le Brigant
    ...
    def reparhor_R3(self, c0, phi0, gamma_f):
        '''
        by Alice Le BRIGANT
    ...
    def geodshoothor_R3(self, ci, cf, m, p, Seuil):
        global tt, tt0, cc
        n = len(ci[0,:])-1
        N = 10
        Cf0 = redisc(cf,p)
        ...
        while ecart_courbes > Seuil :
            ...
            ch, knew = self.reparhor_R3(c, phi, Cf)
            ...
            _ , dist_L2 = self.geod_L2_R3( cf, cf_old, N); 
...
class Curve(RiemannianMetric):
    """
    Curve Class
    """
    def __init__(self,points, N=10):
        self.curve = points
        self.N = N
        self.resample(self.N)
        self.m = 10
        self.p = 100 
        self.Seuil = 0.001
    ...
    def resample(self,N):
        self.curve = ReSampleCurve(self.curve,N)

    def calculate_optimal_matching(self, curve):
        # resample the second curve 
        curve.resample(self.N)
        return self.geodshoothor_R3(self.curve, curve.curve, self.m, self.p, self.Seuil)
...
points1 = data1[start:end,:].T
points2 = data2[start:end,:].T
points2 = np.flip(points2,1)

# create curves from Cruve Class
N =10
c1 = Curve(points1,N)
c2 = Curve(points2,N)

# Calculate Optimal Matching from c1 to c2
Ch, Lh, chemin = c1.calculate_optimal_matching(c2)

# Calculate Optimal Matching from c2 to c1
Ch, Lh, chemin = c2.calculate_optimal_matching(c1)